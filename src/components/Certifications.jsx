  ...
  return (
    <section id="certifications" className="py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionTitle 
          title="AI Certifications & Engineering Journey" 
          subtitle="Featured Anthropic certifications and professional achievements in AI engineering"
        />

        {/* Featured Anthropic row */}
        <div className="flex gap-4 mb-8">
          {certifications.filter(c => c.issuer === 'Anthropic').map((a, i) => (
            <motion.div key={i} className="p-4 bg-white/4 rounded-lg border border-white/10 flex-1" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} viewport={{ once: true }}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  <div className="text-sm font-medium">{a.title}</div>
                  <div className="text-xs text-gray-400">{a.issuer} • {a.year}</div>
                </div>
                <div className="text-sm text-green-400">✔ Verified</div>
              </div>
              <div className="text-sm text-gray-300">{a.credential}</div>
            </motion.div>
          ))}
        </div>
