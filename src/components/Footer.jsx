...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#0A0A0F]/50 backdrop-blur-lg border-t border-primary">
          ...
          >
            <nav className="text-center">
              <ul className="flex flex-col md:flex-row md:gap-6 items-center justify-center">
                {['Projects', 'Research', 'Certifications', 'About'].map((link) => (
                  <li key={link}>
                    <a href={`#${link.toLowerCase()}`} className="text-gray-300 hover:text-white transition-colors duration-200 text-sm px-2">{link}</a>
                  </li>
                ))}
              </ul>
            </nav>
